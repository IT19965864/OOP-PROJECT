...
public class SellerRegservlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String CompanyMobile=request.getParameter("cmobile");
			...
			if(result==true)
			{
				RequestDispatcher dis=request.getRequestDispatcher("abc.jsp");
				dis.forward(request,response);
			}
			else
			{
				resultx=userDButill.add(firstName,lastName,username,email,mobile,address,postalcode,companyName,CompanyAddress,CompanyMobile,password);
				if(resultx==true)
				{
				...
				{
					RequestDispatcher dis=request.getRequestDispatcher("rereg.jsp");
					dis.forward(request,response);
